 ...
 */
export const sendFriendRequest = async (friendId: string): Promise<{ success: boolean; error?: string }> => {
  try {
 ...
 */
export const acceptFriendRequest = async (requestId: string): Promise<{ success: boolean; error?: string }> => {
  try {
    const { error } = await supabase
      .from('friends')
      .update({
 ...
 */
export const rejectFriendRequest = async (requestId: string): Promise<{ success: boolean; error?: string }> => {
  try {
    const { error } = await supabase
      .from('friends')
      .delete()
  ...
  try {
    ...
    return users.map(u => {
      const friendship = friendships?.find(
        f => (f.user_id === user.id && f.friend_id === u.id) ||
      ...
      if (friendship) {
        if (friendship.status === 'accepted') {
          ...
          friendshipStatus = friendship.user_id === user.id ? 'pending' : 'incoming_request';
 ...
 */
export const getMyRankingOnFriendsList = async (friendId: string): Promise<MyRankingInfo | null> => {
  try {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return null;
 ...
 */
export const cancelFriendRequest = async (requestId: string): Promise<{ success: boolean; error?: string }> => {
  try {
    const { error } = await supabase
      .from('friends')
      .delete()
